...
#model = pickle.load(open("lightboost_model.pkl", "rb"))
#model = pickle.load(open("cat_model.pkl", "rb"))
model = pickle.load(open("2020_xgboost.pkl", "rb"))
...
st.markdown(
    ...
    <div style='display: flex; justify-content: space-between; align-items: center;'>
        ...
        <a href='https://public.tableau.com/views/Taxi_Fare_Prediction_Data_2020/NYCTaxiAnalyticsDashboard?:language=en-US&publish=yes&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link' target='_blank' style='text-decoration: none; font-size: 16px; color: #4A90E2;'>📊 Open Tableau Dashboard</a>
    </div>
    """,
...
if st.button("Predict Fare"):
    if pickup != drop and distance is not None:
        ...
        if insert_prediction(time_encoded, datetime_input, pickup, drop, distance, fare):
            st.success(f"Estimated Fare: ${fare:.2f} (saved to database)")
        else:
